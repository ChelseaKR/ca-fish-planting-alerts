...
final class AlertPlannerTests: XCTestCase {
    ...
    func testNewWeekNotifiesOnce() {
        ...
        // Seed the baseline at favouriting time: the current week is on screen already.
        let seeded = AlertPlanner.seeding(AlertState(), favouriting: "cdfw-1", snapshot: snap1)
        let plan1 = AlertPlanner.plan(snapshot: snap1, favourites: ["cdfw-1"], state: seeded)
        XCTAssertEqual(plan1.notifications.count, 0, "what's already visible at favourite time must not notify")
        ...
        let plan2 = AlertPlanner.plan(snapshot: snap2, favourites: ["cdfw-1"], state: plan1.state)
    ...
    func testSameWeekNeverNotifiesTwice() {
        ...
        let seeded = AlertPlanner.seeding(AlertState(), favouriting: "cdfw-1", snapshot: snapshot)

        let plan1 = AlertPlanner.plan(snapshot: snapshot, favourites: ["cdfw-1"], state: seeded)
        ...
        let plan2 = AlertPlanner.plan(snapshot: snapshot, favourites: ["cdfw-1"], state: plan1.state)
        XCTAssertEqual(plan2.notifications.count, 0, "the same week must never notify twice")

        let plan3 = AlertPlanner.plan(snapshot: snapshot, favourites: ["cdfw-1"], state: plan2.state)
        XCTAssertEqual(plan3.notifications.count, 0)
    }

    func testUnfavouritedWaterNeverNotifies() {
        let water = TS.water(id: "cdfw-1", plants: [TS.plant("2026-09-20")])
        let snapshot = TS.snapshot(sourceWeek: "2026-09-20", waters: [water])
        let plan = AlertPlanner.plan(snapshot: snapshot, favourites: [], state: AlertState())
        XCTAssertEqual(plan.notifications.count, 0)
        XCTAssertTrue(plan.state.lastListed.isEmpty)
    }

    func testPastPlantsNeverNotify() {
        // A water favourited today whose only plant is in the past (before source_week).
        let water = TS.water(id: "cdfw-1", plants: [TS.plant("2026-08-01")])
        let snapshot = TS.snapshot(sourceWeek: "2026-09-13", waters: [water])
        let seeded = AlertPlanner.seeding(AlertState(), favouriting: "cdfw-1", snapshot: snapshot)
        let plan = AlertPlanner.plan(snapshot: snapshot, favourites: ["cdfw-1"], state: seeded)
    ...
    func testRemovedThenRelistedNotifiesAgain() {
        // Snapshot A: listed. Favourite (seeded at nothing yet, before it appears)...
        let base = AlertState()
        ...
        let planA = AlertPlanner.plan(snapshot: snapA, favourites: ["cdfw-1"], state: base)
        ...
        let planB = AlertPlanner.plan(snapshot: snapB, favourites: ["cdfw-1"], state: planA.state)
        ...
        let planC = AlertPlanner.plan(snapshot: snapC, favourites: ["cdfw-1"], state: planB.state)
    ...
    func testMultipleNewWeeksInOneRefreshProduceOneNotificationNamingBoth() {
        ...
        let plan = AlertPlanner.plan(snapshot: snap2, favourites: ["cdfw-1"], state: seeded)
    ...
    func testNotificationIdentifierIsDeterministic() {
        let water = TS.water(id: "cdfw-1", plants: [TS.plant("2026-09-13"), TS.plant("2026-09-20")])
        let snapshot = TS.snapshot(sourceWeek: "2026-09-20", waters: [water])
        let planA = AlertPlanner.plan(snapshot: snapshot, favourites: ["cdfw-1"], state: AlertState())
        let planB = AlertPlanner.plan(snapshot: snapshot, favourites: ["cdfw-1"], state: AlertState())
